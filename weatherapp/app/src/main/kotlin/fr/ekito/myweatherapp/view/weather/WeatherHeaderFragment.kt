...
class WeatherHeaderFragment : Fragment() {
    ...
    private val viewModel: WeatherContract.ViewModel by sharedViewModel<WeatherViewModel>()
    ...
    private fun listenToStates() {
        this.viewModel.states.observe(this, Observer {
            when (it) {
                is WeatherViewModel.WeatherListLoaded -> showWeather(it.location, it.first)
            }
        })
    }
    ...
    private fun listenToEvents() {
        this.viewModel.events.observe(this, Observer {
            when(it) {
                is WeatherViewModel.ProceedLocation -> showLoadingLocation(it.location)
                is WeatherViewModel.ProceedLocationFail -> showLocationSearchFailed(it.location, it.error)
            }
        })
    }
